...
<div class="listViewEntriesDiv contents-bottomscroll">
	...
	<table class="table table-bordered listViewEntriesTable">
		...
		<tr class="listViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}' data-recordUrl='{$LISTVIEW_ENTRY->getDetailViewUrl()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
			...
			{if $LISTVIEW_HEADER->get('name') eq 'enabled' and $LISTVIEW_ENTRY->get('checked') eq '1'}
			<td class="listViewEntryValue {$WIDTHTYPE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}" nowrap>
			<img src="{'Enable.png'|vimage_path}" title="{vtranslate('LBL_SQL_CONNECTION_OK', $MODULE )}" style="height:13px;margin-right:5px;vertical-align: middle;">
			{else if $LISTVIEW_HEADER->get('name') eq 'enabled' and $LISTVIEW_ENTRY->get('checked') eq '0'}
			<td class="listViewEntryValue {$WIDTHTYPE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}" nowrap>
			<img src="{'Disable.png'|vimage_path}" title="Bad connection, please repair" style="height:13px;margin-right:5px;vertical-align: middle;">			
			{else}
			<td class="listViewEntryValue {$WIDTHTYPE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}" nowrap>
			{/if}
				...
				{else if $LISTVIEW_HEADER->get('uitype') eq '99'}
					{assign var="j_dbpassword" value='*****' nocache} {$j_dbpassword}				
...
<div class="span8 alert alert-danger container-fluid">
    ...
    {vtranslate('LBL_JOOMLAHOSTS_INFO', $QUALIFIED_MODULE)}
</div>	
{/strip}
