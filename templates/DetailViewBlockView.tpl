...
{strip}
	...
	<table class="table table-bordered equalSplit detailview-table">
		...
		{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
			...
			{else}
				...
				{if $FIELD_MODEL->get('uitype') eq '99'}
				 <td class="fieldValue {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}">&nbsp;* * * * </td>
				{else}
	...
	{/foreach}
{if $CHECKED eq '1'}
<div class="span6 alert alert-info container-fluid">
	{vtranslate('LBL_JOOMLAHOSTS_CHECKED', $QUALIFIED_MODULE)}
</div>
{else}
<div class="span8 alert alert-danger container-fluid">
    <b>{vtranslate('LBL_JOOMLAHOSTS_NOSQL', $QUALIFIED_MODULE)}</b>
</div>
{/if}
{/strip}